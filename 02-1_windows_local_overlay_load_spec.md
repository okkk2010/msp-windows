...
- 컨트롤: `ComboBox loadedOverlayList`
- 표시 형식: `Code | Title`
- 동작:
  - Code Load 성공 시 해당 항목을 목록에 추가
  - 동일 code가 이미 있으면 title/overlayId를 갱신하고 해당 항목을 선택
  - 목록은 선택만 제공하며, 현재 단계에서는 선택으로 렌더링 적용을 수행하지 않음

### 3.5 오버레이 코드 입력/조회 (서버 조회)

- 컨트롤: `TextBox overlayCodeTextBox`, `Button loadByCodeButton`, `Label codeLoadStatusLabel`
- code 정책:
  - 입력값 trim 후 대문자 변환
  - `^[A-Z0-9]{6}$` 형식이 아니면 요청 차단
- 동작:
  - `GET http://localhost:8080/api/overlays/code/{code}` 호출
  - 응답 `success`, `data`를 확인
  - 성공 시 `loadedOverlayList`에 `Code | Title` 항목 추가/선택
  - 실패 시 상태 라벨 및 오류 로그(`E210`)에 메시지 표시
  - 현재 단계에서는 overlayJson 파싱/적용/캐시 저장을 수행하지 않음
...
4. `오버레이 코드 입력` 후 `Code Load` 클릭 시 서버에서 오버레이 메타데이터(code/title) 조회
5. 조회 성공 시 `불러온 오버레이 (Code | Title)` 목록에서 항목 선택
